...
class SyntheticDataset(abc.ABC):
    ...
    def load(self, text_path, t_start, t_end):
        ...
        his_s = torch.tensor(his_st[:, :2], dtype=torch.float32, device=DEVICE)
        his_t = torch.tensor(his_st[:, 2], dtype=torch.float32, device=DEVICE)

        idx = (his_t >= t_start) & (his_t < t_end)
        self.his_s = his_s[idx]
        self.his_t = his_t[idx]

        if isinstance(self, DEBMDataset):
            self.his_t[0] -= eps

    def dataset(self, lookback=10, lookahead=1, split=None):
        # Convert to torch and compute deltas
        his_s = torch.tensor(self.his_s, dtype=torch.float32, device=DEVICE) \
            if not torch.is_tensor(self.his_s) else self.his_s
        his_t = torch.tensor(self.his_t, dtype=torch.float32, device=DEVICE) \
            if not torch.is_tensor(self.his_t) else self.his_t

        if self.dist_only:
            delta = his_s[1:] - his_s[:-1]
            dist = torch.norm(delta, dim=1)
            dist = torch.cat((torch.tensor([0.], device=DEVICE), dist)).view(-1, 1)
            st_data = torch.cat((dist, his_t.view(-1, 1)), dim=1)
        else:
            st_data = torch.cat((his_s, his_t.view(-1, 1)), dim=1)

        # Compute delta_t
        delta_t = torch.cat((torch.tensor([0.], device=DEVICE), his_t[1:] - his_t[:-1]))
        st_data[:, -1] = delta_t
        ...
        st_input = torch.stack([st_data[i:i + lookback] for i in range(length)])
        st_label = torch.stack([st_data[i + lookback:i + lookback + lookahead] for i in range(length)])
    ...
    def get_lamb_st(self, x_num, y_num, t_num, t_start, t_end):
        his_t = self.his_t[(self.his_t >= t_start) & (self.his_t < t_end)]
        his_s = self.his_s[(self.his_t >= t_start) & (self.his_t < t_end)]

        x_min, y_min = torch.min(his_s, dim=0).values.tolist()
        x_max, y_max = torch.max(his_s, dim=0).values.tolist()
        ...
        for t in tqdm(t_range):
            lamb_st = torch.zeros((x_num, y_num), device=DEVICE)
            for i, x in enumerate(x_range):
                for j, y in enumerate(y_range):
                    lamb_st[i, j] = self.lamb_st(torch.tensor([[x, y]], device=DEVICE), t)
            lambs.append(lamb_st.cpu().numpy())  # convert to numpy for downstream usage
...
class STSCPDataset(SyntheticDataset):
    ...
    def stoi(self, s):
        s_idx = torch.round(s * torch.tensor([[self.x_num - 1, self.y_num - 1]], device=DEVICE))
        return (s_idx @ torch.tensor([[1], [self.y_num]], device=DEVICE)).long().squeeze()
    ...
    def generate_batch(self, t_start, t_num, mu, verbose=False):
        ...
        while True:
            lamb_t_val = self.lamb_t(mu, his_s, t)
            l = 2 / lamb_t_val
            m = lamb_t_val * torch.exp(self.beta * l)
            ...
            if delta_t > l:
                ...
                if new_lamb_t / m >= torch.rand(1, device=DEVICE).item():
                    ...
                    his_t = torch.cat((his_t, torch.tensor([t], device=DEVICE)), dim=0)

        for t_eval in t_range:
            lambs.append(self.lamb_St(mu, his_s[his_t < t_eval], t_eval))
...
class STHPLearner:
    ...
    def train(self, train_s, train_t, t_start=None, t_end=None):
        self.dset.his_s = torch.tensor(train_s, dtype=torch.float32, device=DEVICE)
        self.dset.his_t = torch.tensor(train_t, dtype=torch.float32, device=DEVICE)
...
class DEBMDataset(SyntheticDataset):
    def __init__(self, s_mu, g2_cov, alpha, beta, mu, max_history=100, dist_only=False):
        super().__init__(dist_only)
        self.s_mu = torch.tensor(s_mu, dtype=torch.float32, device=DEVICE)
        self.g2_cov = torch.tensor(g2_cov, dtype=torch.float32, device=DEVICE)
        self.alpha = alpha
        self.beta = beta
        self.mu = mu
    ...
    def generate(self, t_start, t_end, verbose=False):
        t = 0
        self.his_s = self.s_mu.view(1, 2)
        self.his_t = torch.tensor([-eps], dtype=torch.float32, device=DEVICE)
        ...
        while True:
            ...
            if delta_t > l:
                ...
                if new_lamb_t / m >= torch.rand(1, device=DEVICE).item():
                    ...
                    s = torch.tensor(s, dtype=torch.float32, device=DEVICE).view(1, 2)
                    self.his_s = torch.cat((self.his_s, s), dim=0)
                    self.his_t = torch.cat((self.his_t, torch.tensor([t], dtype=torch.float32, device=DEVICE)))
