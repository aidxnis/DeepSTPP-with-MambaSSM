...
class SyntheticDataset(abc.ABC):
    ...
    # def g2(s, his_s, s_sqrt_inv_det_cov, s_inv_cov):
    #     # Ensure s and his_s are broadcastable
    #     s = s.unsqueeze(1)  # Shape becomes [x_num * y_num, 1, 2]
    #     his_s = his_s.unsqueeze(0)  # Shape becomes [1, N, 2]

    #     # Now, we can safely subtract s and his_s element-wise
    #     delta_s = s - his_s  # Shape: [x_num * y_num, N, 2]
        
    #     # Compute the exponent (Mahalanobis distance)
    #     exponent = -0.5 * torch.sum((delta_s @ s_inv_cov) * delta_s, dim=-1)
    ...
    def g2(s, his_s, s_sqrt_inv_det_cov, s_inv_cov):
        ...
        # Return the kernel value and ensure correct shape
        g2_vals = (1 / (2 * np.pi)) * s_sqrt_inv_det_cov * torch.exp(exponent)

        # Only squeeze if the last dimension is size 1 (i.e., for single-value scenarios)
        if g2_vals.shape[-1] == 1:
            return g2_vals.squeeze(-1)
        else:
            return g2_vals 
    ...
    def get_lamb_st(self, x_num, y_num, t_num, t_start, t_end):
        ...
        x_max, y_max = torch.max(his_s, dim=0)[0]
        x_min, y_min = torch.min(his_s, dim=0)[0]
        ...
        # Generate a grid of shape (x_num * y_num, 2)
        gx, gy = torch.meshgrid(x_range, y_range, indexing='ij')
        s_grids = torch.stack([gx.flatten(), gy.flatten()], dim=1).to(DEVICE)

        lambs = []
        for t in tqdm(t_range, desc="Evaluating λ(s,t)"):
            # Vectorized computation for all points in the grid at once
            lamb_st = self.lamb_st(s_grids, t)  # Pass the 2D grid of points to lamb_st
            lambs.append(lamb_st.view(x_num, y_num).T.cpu().numpy())
...
class STSCPDataset(SyntheticDataset):
    ...
    def generate(self, t_start, t_end, t_num=None, verbose=False):
        ...
        self.lambs = [self.mu * torch.ones(self.x_num * self.y_num, device=DEVICE)]
        ...
        while self.t_range[-1] < self.t_end:
            ...
            self.his_s = torch.cat((self.his_s, his_s), dim=0)
            self.his_t = torch.cat((self.his_t, his_t), dim=0)

        self.lambs = [lamb_st.view(self.x_num, self.y_num).T.cpu().numpy() for lamb_st in self.lambs]
    ...
    def generate_batch(self, t_start, t_num, mu, verbose=False):
        ...
        t = 0
        his_s = torch.zeros((0, 2), device=DEVICE)
        his_t = torch.tensor([], device=DEVICE)

        t_range = torch.linspace(0, t_end, t_num, device=DEVICE)[1:]
        lambs = []
        ...
        while True:
            ...
            if delta_t > l:
                ...
                if new_lamb_t / m >= torch.rand(1, device=DEVICE).item():
                    ...
                    i = torch.argmax(torch.multinomial(p, 1))
                    his_s = torch.cat((his_s, self.s_grids[i].view(1, 2)), dim=0)
                    his_t = torch.cat((his_t, torch.tensor([t], device=DEVICE)))
...
class STHPDataset(SyntheticDataset):
    def __init__(self, s_mu, g0_cov, g2_cov, alpha, beta, mu, max_history=100, dist_only=False):
        ...
        # Compute inverses and determinants on the GPU
    ...
    def trunc(self, his):
        """ Truncate history if length exceeds max_history """
        return his[-self.max_history:] if len(his) > self.max_history else his

    def lamb_st(self, s, t):
        """ Calculate λ(s,t|H) """
        s = s.to(DEVICE, dtype=torch.float32) if not torch.is_tensor(s) else s.to(DEVICE)
        ...
        if len(his_t) == 0:
            excitation = torch.tensor(0.0, device=DEVICE)
        else:
    ...
    def predict_next(self, i):
        ti = self.his_t[i].to(DEVICE)
        hist_t_trunc = self.trunc(self.his_t[:(i+1)])
        c = self.alpha * torch.sum(torch.exp(-self.beta * (ti - hist_t_trunc)))

        def int_lamb(t):
            return torch.exp(c / self.beta * (torch.exp(-self.beta * (t - ti)) - 1) - self.mu * (t - ti))

        def time_pdf(t):
            return t * (self.mu + c * torch.exp(-self.beta * (t - ti))) * int_lamb(t)

        def space_pdf(t):
            hist_s_trunc = self.trunc(self.his_s[:(i+1)])
            spatial_sum = torch.sum(torch.exp(-self.beta * (t - hist_t_trunc))[:, None] * hist_s_trunc, dim=0)
            return (self.mu * self.s_mu + self.alpha * spatial_sum) * int_lamb(t)

        t_range = torch.linspace(ti.item(), 1000, steps=1000, device=DEVICE)
        time_integral = torch.trapz(time_pdf(t_range), t_range)
        space_integral = torch.trapz(space_pdf(t_range), t_range)

        return space_integral.item(), time_integral.item()


    def generate_offsprings(self, t_i, s_i, verbose=False):
        """ Generate offsprings for event i using Ogata thinning """
        t = t_i
        count = 0
        while True:
    ...
    def generate(self, t_start, t_end, verbose=False):
        """ Generate events in [0, T] with self-exciting process """
        self.t_start = t_start
        self.t_end = t_end
        t = t_start
    ...
    def plot_intensity(self, s=None, t_start=None, t_end=None, color='blue'):
        """ Plot the intensity function and event distances """
        if s is None:
