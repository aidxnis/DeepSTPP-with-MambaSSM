...
def calc_lamb(model, test_loader, config, device, scales=np.ones(3), biases=np.zeros(3),
    ...
    for data in test_loader:
        ...
        mask = idx == 0
        st_xs.append(st_x[mask].to(device))
        st_ys.append(st_y[mask].to(device))
        st_x_cums.append(st_x_cum[mask].to(device))
        st_y_cums.append(st_y_cum[mask].to(device))
    ...
    st_x_cum = torch.cat(st_x_cums, 0)
    st_y_cum = torch.cat(st_y_cums, 0)
    ...
    # Calculate intensity
    background = model.background.unsqueeze(0).repeat(st_x.size(0), 1, 1).to(device)

    # Sample model parameters
    _, w_i, b_i, inv_var = model(st_x)

    # Convert to history for return
    his_st = torch.vstack((st_x[0].cpu(), st_y.squeeze().cpu())).numpy()
    his_st_cum = torch.vstack((st_x_cum[0].cpu(), st_y_cum.squeeze().cpu())).numpy()

    for t in tqdm(t_range):
        i = sum(st_x_cum[:, -1, -1] <= t) - 1

        st_x_ = st_x[i:i+1]
        w_i_ = w_i[i:i+1]
        b_i_ = b_i[i:i+1]
        inv_var_ = inv_var[i:i+1]
        background_ = background[i:i+1]
        ...
        # Spatial intensity
        N = len(s_grids)
        s_x_ = torch.cat((st_x_[..., :-1], background_), 1).repeat(N, 1, 1)
        ...
        lamb_s = s_intensity(
            ...
            t_ti.repeat(N, 1),
            s_diff,
            inv_var_.repeat(N, 1, 1)
        )

        lamb = (lamb_s * lamb_t).view(x_nstep, y_nstep)
        lambs.append(lamb.cpu().numpy())
