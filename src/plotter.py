...
def inverse_transform(xg, yg, tg, scaler):
    """
    GPU tensors in, GPU tensors out. Preserves shape. Applies inverse_transform independently.
    """
    x_arr = torch.stack([xg.cpu(), torch.zeros_like(xg.cpu()), torch.zeros_like(xg.cpu())], dim=1)
    y_arr = torch.stack([torch.zeros_like(yg.cpu()), yg.cpu(), torch.zeros_like(yg.cpu())], dim=1)
    t_arr = torch.stack([torch.zeros_like(tg.cpu()), torch.zeros_like(tg.cpu()), tg.cpu()], dim=1)

    x_inv = scaler.inverse_transform(x_arr)[:, 0]
    y_inv = scaler.inverse_transform(y_arr)[:, 1]
    t_inv = scaler.inverse_transform(t_arr)[:, 2]

    return (
        torch.tensor(x_inv, device=xg.device),
        torch.tensor(y_inv, device=yg.device),
        torch.tensor(t_inv, device=tg.device),
    )
...
def plot_lambst_static(
    ...
    assert X.shape == lambs[0].shape, f"Shape mismatch: X={X.shape}, lambs[0]={lambs[0].shape}"
